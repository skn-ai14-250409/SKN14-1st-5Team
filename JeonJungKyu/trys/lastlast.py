...
# 출발지, 경유지, 목적지 드롭다운에 "지역을 선택해주세요" 추가
...
# 드롭박스에서 "지역을 선택해주세요"를 선택했을 때는 아무 동작하지 않도록 처리
def handle_news_selection(selected_location, table_name):
    if selected_location != "지역을 선택해주세요":  # "지역을 선택해주세요"가 선택되지 않으면
...
def process_news_from_db(table_name: str):
    ...
    if not urls:
        ...
        seen_urls = set()  # 이미 처리된 뉴스의 originallink를 추적하는 집합
        for idx, url in enumerate(urls, start=1):
            if url in seen_urls:
                continue  # 이미 처리된 뉴스는 건너뜁니다

            seen_urls.add(url)  # 새로운 뉴스의 originallink를 집합에 추가
...
# 드롭박스 값에 따라 뉴스 처리
handle_news_selection(departure, "departure_news")
handle_news_selection(transit, "transit_news")
handle_news_selection(destination, "destination_news")
